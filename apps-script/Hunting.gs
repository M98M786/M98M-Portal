...
/* 25 Sept (owner): "it is compulsory to add all the data other than Temu link" for New and
   Seasonal items; only a Duplicated item may travel light (the five core fields above). The
   hunter's own Comment and the CPC choice stay optional — the CPC type is Management's call at
   approval, and always was. */
const HUNT_FULL_SET = [HC_KEYWORD, HC_TITLE, HC_CATEGORY, 'Terapeak overview', 'Terapeak Second Page',
  'Image Link of Zik analytics', 'Image Link of avg sold price', HC_KEYWORD_COMPETITORS, 'Sell Through',
  'Sold Unit ANALYSIS', 'Price Range ANALYSIS', HC_TOP_THREE, 'Ebay Link', 'Prime Competitor Selling Price',
  'High Price Link', 'Competitor High Selling Price', 'Competitors', HC_SUPPLIER_1, 'Product Link 2',
  'Product Link 3', HC_SOURCE_PRICE, HC_CALC_PRICE, 'Image Link', 'DESCRIPTION'];
...
const HUNT_SHEET_HEADERS = ['Selected By ', 'Approval Status', 'Comments', 'Date Added ',
  ...
  'Competitor High  Selling Price', 'Campaign category', 'Terapeak Second Page'];
...
/* 25 Sept (owner): "make three departments — New items, Duplicated items, Seasonal items". The
   same column carries all three: Consistent IS the New-item department (its historical name),
   Duplicated is an item already live on one of our accounts being copied to another. */
const HUNT_KINDS = ['Seasonal', 'Consistent', 'Duplicated'];
const HC_KIND = 'Seasonal';
function huntKind_(value, required) {
  const s = String(value === null || value === undefined ? '' : value).trim();
  if (!s) { if (required) throw new Error(SAFE_ERROR_PREFIX + 'choose the hunting department: New (Consistent), Seasonal or Duplicated'); return ''; }
  for (let i = 0; i < HUNT_KINDS.length; i++) {
    if (huntNormKey_(HUNT_KINDS[i]) === huntNormKey_(s)) return HUNT_KINDS[i];
  }
  throw new Error(SAFE_ERROR_PREFIX + 'the item type must be Consistent (new), Seasonal or Duplicated');
}
...
function actionSubmitHunt_(payload, ctx) {
  ...
  /* Write-path shadow (Phase 1 / WO-2, parity: parity/hunt-submit.md): rehearse this exact submit on
     the ENGINE — it re-runs the §3 validation + §4 dup-check against the hunt_rows mirror and records
     the verdict it WOULD have produced, writing NOTHING else (no sheet, no task, no notification).
     Placed first so a submission the old path is about to reject is still shadowed. Best-effort and
     gated by config; the old path below is byte-for-byte unchanged. Off unless write_hunt_mode=shadow. */
  try {
    if (getConfig('write_hunt_mode') === 'shadow') {
      enginePost_('shadowHuntSubmit', { columns: (payload && payload.columns) || {},
        shipping: (payload && payload.shipping) || '', override_note: (payload && payload.override_note) || '',
        caller: ctx.ident.email, role: ctx.user.role });
    }
  } catch (e) { /* the shadow can never affect the real submit */ }
  ...
  cols[HC_KIND] = huntKind_(cols[HC_KIND], true);   // the department — required at submit

  /* 25 Sept (owner): New (Consistent) and Seasonal items carry the FULL research set — every
     field except the Temu link; Duplicated items may skip. Management with an override note can
     still push a partial row through (the same override that clears a duplicate). */
  if (cols[HC_KIND] !== 'Duplicated' &&
      !(String(payload.override_note || '').trim() && isMgmt_(ctx.user.role, ctx.ident.email))) {
    const fsMissing = HUNT_FULL_SET.filter(function (c) { return String(cols[c] || '').trim() === ''; });
    if (fsMissing.length) {
      throw new Error(SAFE_ERROR_PREFIX + 'a ' + (cols[HC_KIND] === 'Seasonal' ? 'Seasonal' : 'New') +
        ' item needs the full research — still empty: ' + fsMissing.slice(0, 5).join(', ') +
        (fsMissing.length > 5 ? ' + ' + (fsMissing.length - 5) + ' more' : '') +
        '. Only the Temu link is optional; a Duplicated item may travel light.');
    }
  }
...
function actionDecideHunt_(payload, ctx) {
  ...
  if (decision === HUNT_REVISION) {
    ...
    try {
      ...
      if (rrec.approval_status !== HUNT_PENDING) {
        ...
        if (String(rrec.approval_status) === String(HUNT_REVISION)) { return { revision: true, hunt_id: rrec.hunt_id, idempotent: true }; }
        throw new Error(SAFE_ERROR_PREFIX + 'this hunt is already ' + String(rrec.approval_status).toLowerCase() + ' — press Refresh on the queue');
      }
      const rpatch = {};
  ...
  if (taskId) engineTaskPush_(taskId, (typeof taskRowOut !== 'undefined' && taskRowOut) ? taskRowOut : null);   // the lister's new task on the boards in seconds — no re-read of TASKS
...
function huntFind_(sh, huntId) {
  ...
  /* 13 Sept (owner: "make the speed fast"): every hunt action opened with a read of the WHOLE
     HUNTING_DB (every column of every hunt) to locate one row by id. Scan column A, then read
     that one row — the same answer for a fraction of the transfer. */
  const lr = sh.getLastRow();
  if (lr < 2) throw new Error('hunt not found');
  ...
  for (let i = 0; i < ids.length; i++) {
    ...
    const rec = {};
    head.forEach(function (h, c) { rec[h] = rowVals[c]; });
    return { row: i + 2, head: head, rec: rec };
  }
...
/* allow: the column whitelist for THIS write — defaults to the decision columns (management's
   pen). actionReviseHunt_ passes the hunter-editable set instead: the default walled the hunter's
...
/* 25 Sept migration — the one genuinely NEW column the owner's 37-column workflow needs:
 * 'Terapeak Second Page' (#9, Market Research). Appends the header to the Portal DB HUNTING_DB
 * tab and to the live hunting workbook's own header row; both reads are header-addressed, so
 * the cell is all either side needs. Run once from the editor; idempotent. */
function huntAddSecondTerapeakCol() {
  const out = [];
  const NEW_COL = 'Terapeak Second Page';
  const db = huntSheet_();
  ...
  try {
    const id = bridgeResolveSheetId_('global', '', 'hunting');
    if (!id) { out.push('live workbook: not connected'); return out.join(' · '); }
    const open = bridgeOpenTab_(id, HUNT_SHEET_TABS, HUNT_SHEET_HEADERS);
  ...
  logActivity_('system', 'HUNT_COL_ADD', 'Terapeak Second Page', '', '', out.join(' · ').slice(0, 200));
  return out.join(' · ');
}
...
function actionReviseHunt_(payload, ctx) {
  ...
  try {
    ...
    /* The hunter-editable whitelist: every hunting column EXCEPT the ones that are not theirs to
       touch — who selected it, when it arrived, the =IMAGE cell, and the two management-only
       routing columns (already deleted from `sent` above; forbidden here too, belt and braces). */
    const reviseAllow = HUNTING_COLS.filter(function (c) {
      return [HC_SELECTED_BY, HC_DATE_ADDED, HC_IMAGE, HC_ACCOUNT, HC_LISTING_STATUS].indexOf(c) < 0;
    });
    huntWrite_(sh, found, sent, reviseAllow);
  } finally { lock.releaseLock(); }
...
const HUNT_RECORDS_ROLES = ['Product Hunter', 'Team Lead', 'Ops Head', 'Management',
  'Order Processor', 'Advertising Manager', 'Listing Manager', 'Sales Operations', 'Item Lister'];
