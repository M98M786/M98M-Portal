...
const ENGINE_RUNNABLE = {
  ...
  notifTail: function (args) { return notifTail(args); },
  notifCursorFix: function (args) { return notifCursorFix(args); },
...
/* ============ Listing-ladder task bridge (owner, 10 Sept) ============
 * The engine decides WHAT happens on the ladder (72h / 10-day / 20-day, tiers, video, finals);
 * THIS creates the actual task on the board — through the same routing law as every revision:
 * the two-week desk override first, then the lister who listed it, then the role fallback.
 * kind 'revision' → listing_revision task · kind 'video' → the video step ·
 * kind 'end' → a general end-listing task for the Team Lead. */
function ladderTaskCreate_(a) {
  a = a || {};
  const item = String(a.item_id || '').trim();
  const account = String(a.account || '').trim();
  const title = String(a.title || '').trim();
  const stageRaw = String(a.stage || '').trim();
  ...
  if (kind === 'end') {
    const tl = listingPickForRole_('Team Lead', '', all, 'general') || listingPickForRole_('Listing Manager', '', all, 'general');
    if (!tl) throw new Error('ladderTask: nobody holds Team Lead');
    const tid = listingCreateTask_(tasksSheet_(), {
      type: 'general', account: account, item_id: item,
  ...
  /* revision + video route to the lister side, under the same law as every revision task */
  const ovr = (typeof listingRevisionOverride_ === 'function') ? listingRevisionOverride_() : '';
  let to = ovr || String(a.lister_email || '').trim();
  if (!to) {
    const pick = listingPickForRole_('Item Lister', '', all, 'listing_revision') || listingPickForRole_('Listing Manager', '', all, 'listing_revision');
    to = pick ? pick.email : '';
  }
  if (!to) throw new Error('ladderTask: nobody to assign');
  const isVideo = kind === 'video';
  const lines = [
    'Item ID: ' + item, title ? 'Listing: ' + title : '',
    'Ladder stage: ' + stageLabel + (tier ? ' · Tier ' + tier.replace('T', '') : ''),
    String(a.comment || ''),
  ];
  if (String(a.title_keywords || '').trim()) lines.push('TITLE KEYWORDS (from the Advertising Manager): ' + String(a.title_keywords).trim());
  if (String(a.desc_keywords || '').trim()) lines.push('DESCRIPTION KEYWORDS: ' + String(a.desc_keywords).trim());
  if (isVideo) {
    lines.push('Add a product VIDEO on the eBay listing, then record the video link on the Video revisions page — the task completes there.');
  } else {
    lines.push('On submission: say exactly WHAT you changed, and resubmit the keyword/SEO research data (the research button on this task) — every version is archived.');
    if (tier === 'T2') lines.push('TIER 2: you may press \u201cWaiting for the Advertising Manager\u2019s call\u201d on this task — that parks it until the Advertising Manager calls the revision in.');
  }
  lines.push('[LADDER:' + (stageRaw || 'REV') + ':' + (tier || '-') + ':' + item + ']');
  const tid = listingCreateTask_(tasksSheet_(), {
    type: 'listing_revision', account: account, item_id: item,
    title: (isVideo ? 'Video revision' : stageLabel) + ' — Item ID ' + item,
    details: listingLines_(lines.filter(String)),
    assigned_by: 'system:ladder', assigned_to: to,
    priority: tier === 'T2' && !isVideo ? '' : 'high', deadline_pkt: due, stamp: stamp,
  });
  try { engineTaskPush_(tid); } catch (e) {}
  notify_(to, 'Task assigned', (isVideo ? '🎬 Video revision' : '🟣 ' + stageLabel) + ' · Item ' + item + ' · ' + account +
    (tier ? ' · Tier ' + tier.replace('T', '') : '') + ' — keywords and instructions are on the task.', 'task:' + tid);
  return JSON.stringify({ task_id: tid, to: to, kind: kind });
}
