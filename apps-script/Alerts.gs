...
function actionResolveAlert_(payload, ctx) {
  ...
  if (!isMgmt_(ctx.user.role, ctx.ident.email)) {
    // Owner 9 Sept: account-health alerts go to the CS individuals who work them, with management in the loop.
    metricsNotifyCsMgmt_('Account alert resolved', msg, ALERTS_NOTIFY_REF + alertsKey_(account, fingerprint, want.raised_at));
  }
...
function alertsNotifyFresh_(fresh) {
  ...
  fresh.slice(0, ALERTS_NOTIFY_MAX).forEach(function (f) {
    ...
    metricsNotifyCsMgmt_('Account report ACTIVE alert', msg, f.ref);   // owner 9 Sept: CS individuals + management
  ...
  if (fresh.length > ALERTS_NOTIFY_MAX) {
    ...
    metricsNotifyCsMgmt_('Account report ACTIVE alert',
