...
function pushSheetRowsHot() {
  ...
  var stepT = Date.now(), stepLog = [];
  var stepDone = function (name) { stepLog.push(name + ' ' + Math.round((Date.now() - stepT) / 1000) + 's'); stepT = Date.now(); };
  ...
  logActivity_('system', 'SHEETMIRROR_HOT', '', '', String(pushed), (runBooks
    ? tabs + ' tab(s) · walked ' + (typeof walkedNames !== 'undefined' ? walkedNames.join(', ') : '?') + ' · skipped ' + (typeof skippedNames !== 'undefined' ? skippedNames.join(', ') : '?')
    : 'books walk skipped (15-min throttle)') + ' · steps: ' + stepLog.join(', '));
...
function tasksSweep_() {
  try {
    ...
    /* Sync the most-recently-UPDATED tasks, not the last 100 CREATED. The old slice(-100) mirrored
       only the newest rows BY POSITION, so an edit to an OLDER task — go-live clearing a draft flag
       (enterItemId), a hunter working a revision, a completion — never reached D1, and the
       engine-served desks (go-live desk, hunting dashboard, dept boards) showed the stale state on
       every refresh, forever. enterItemId and every worker action bump updated_at, so sorting by it
       pulls any touched task into the window and it syncs. (owner, 6 Sept — "it says live but comes
       back on refresh · hunting dashboard still not updating".) */
    var mapped = rows.map(function (t) {
...
function adminRenameUser(args) {
  ...
  var colIx = function (want) { for (var k = 0; k < head.length; k++) { if (String(head[k]).trim().toLowerCase() === want) return k; } return -1; };
  var ei = colIx('email'), ni = colIx('name'), ri = colIx('role');
