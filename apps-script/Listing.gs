...
function actionEnterItemId_(payload, ctx) {
  ...
  /* §8 chain — OUTSIDE the lock (8 Sept overload work): the go-live write above already landed
     inside the lock, so the listing is live and off the go-live desk. These three sub-tasks are
     best-effort and idempotent on (type,item_id) via listingFindTask_, and appendRow is atomic —
  ...
  if (rec) {
    ...
    /* ③ +72h revision — RETIRED as a direct task (owner, 10 Sept): the LISTING LADDER owns the
       ...
       (tiered) through ladderTask. Spawning one here too gave every go-live a redundant task. */
    made.revision = { ladder: true };
  }
...
/** Overload tranche 2 (10 Sept, go-live day): the lister-flow finder runs BEFORE the lock now;
 * inside it one row is re-read (taskVerify_) and the same guards re-applied to the FRESH values,
 * so the decision is still made on locked truth — without the full-tab read serialising everyone. */
function listingListerVerify_(sh, pre, payload, ctx, wantOpen) {
  const found = taskVerify_(sh, pre, payload.task_id);
  const rec = found.rec;
  if (String(rec.type || '') !== 'listing_new') throw new Error('not a listing task');
  if (normalizeEmail(rec.assigned_to) !== normalizeEmail(ctx.ident.email) && !isMgmt_(ctx.user.role, ctx.ident.email)) {
...
function actionListerNeedTime_(payload, ctx) {
  ...
  const sh = tasksSheet_();
  let rec = null, pushed = '';
  const pre = listingListerTask_(sh, payload, ctx, true);   // heavy read outside the lock (tranche 2)
  const lock = LockService.getScriptLock();
  try {
    lock.waitLock(10000);
...
function actionListerDraft_(payload, ctx) {
  ...
  /* 11 Sept (owner: "combine the page of draft link and keyword research"). The keyword/SEO
     research now rides WITH the hand-off in one request. Save it to D1 through the sync bridge
     BEFORE the lock (never hold the sheet lock across a network call), then REQUIRE research to
     exist — a draft cannot reach go-live without it. This one reliable step replaces the separate
     browser→engine save that kept dropping and left listers stuck. */
  const resItemKey = 'task:' + String(pre.rec.task_id);
  const research = (payload.research && typeof payload.research === 'object') ? payload.research : null;
  let researchOk = false;
  if (research && Object.keys(research).length) {
    try {
      enginePost_('ladderResearchSaveSync', { item_id: resItemKey, kind: 'LISTING',
        data: research, changes_note: String(payload.changes_note || ''), submitted_by: ctx.ident.email });
      researchOk = true;
    } catch (e) {
      throw new Error(SAFE_ERROR_PREFIX + 'could not save the keyword research just now — press Hand to go-live again in a moment (your typing is kept on this device).');
    }
  } else {
    try { const ex = enginePost_('ladderResearch', { item_id: resItemKey }); researchOk = !!(ex && ex.research && ex.research.length); } catch (e) {}
  }
  if (!researchOk) throw new Error(SAFE_ERROR_PREFIX + 'add the keyword & SEO research on the go-live form before handing it off');
...
function actionListerClearFlag_(payload, ctx) {
  const sh = tasksSheet_();
  let rec = null;
  const pre = listingListerTask_(sh, payload, ctx, false);   // heavy read outside the lock (tranche 2)
  const lock = LockService.getScriptLock();
  try {
    lock.waitLock(10000);
    const found = listingListerVerify_(sh, pre, payload, ctx, false);
    rec = found.rec;
