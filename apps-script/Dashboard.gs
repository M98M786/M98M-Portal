...
function dashMonthCard_(bucket, period, wanted, ctx) {
  ...
  DASH_TILES.concat([DASH_METRIC_RETURNS_OLD]).forEach(function (m) {
    ...
    if (v === null) return;
    /* The fleet total already refuses a broken workbook cell (dashCollective_), but the ACCOUNT
       card printed the same cell raw — which is how a Margin tile of -2,209,167,756,002 kept
       reaching Management's screen after the collective figure had been cleaned. A number this
       far outside trade is a broken formula in the workbook, not a business fact: name the cell
       instead of rendering it, so the screen can say which sheet to fix. */
    if (dashSaneNumber_(v) === null) { broken.push({ metric: m, value: String(v).slice(0, 28) }); return; }
    sheet[m] = v;
  });
  card.sheet_tiles = dashStrip_(sheet, ctx);
  card.sheet_broken = broken;
...
function dashDiscrepancies_(account, card, wanted) {
  ...
  DASH_TILES.forEach(function (metric) {
    ...
    /* Never raise a "these disagree" row against a cell we already know is broken — it produced
       a £2.2tn "difference" and buried the real discrepancies underneath it. */
    if (dashSaneNumber_(mine) === null || dashSaneNumber_(theirs) === null) return;
...
function dashEngineMonthTruth_(curMonth) {
  /* 30 Aug: one engine query beats paging the whole table — and carries orders/units/actual so
     the SCREEN TILES can show eBay's own month, not the lagging cards. Falls back below. */
  try {
    const t = enginePost_('monthTruth', {});
    if (t && t.month === curMonth && t.sold) {
      return { month: curMonth, sold: t.sold, orders: t.orders, units: t.units,
        actual: t.actual, cpc_ads: t.cpc_ads, ads: t.ads,
        per_account: {}, last_fresh_day: t.last_fresh_day || {}, as_of: now_() };
    }
  } catch (e) { /* fall through to the paging path */ }
