...
function actionAccountsAdmin_(payload, ctx) {
  sadmAssertStaffAdmin_(ctx);
  const canEdit = sadmIsManagement_(ctx);           // 13 Sept: Management edits sheet links (was super-only)
...
function actionSaveAccount_(payload, ctx) {
  /* 13 Sept (owner: "Staff & accounts is too complicated and doesn't work at all"): the sheet
     links were super-admin-only, so the owner's everyday Management login saw a read-only screen
     and every save answered 'auth'. Management may link, rename and archive accounts now; role
     and email changes on people stay super-only (unchanged, in actionUpdateStaff_). */
  sadmAssertStaffAdmin_(ctx);
...
const ACTIONS_STAFFADMIN = {
  ...
  saveAccount:     [actionSaveAccount_, 'any'],     // Management gated inside (13 Sept)
};
