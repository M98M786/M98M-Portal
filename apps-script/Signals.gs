...
/* 18 Sept (owner: "advertising, customer service and signals — all signal pages should have
   different signals; they should receive signals related to their own departments"). The CS desk
   had one signal of its own (a per-item returns spike, rare) and was otherwise shown the
   Advertising desk's loss cards. Two CS-only signals now come from the engine's eBay truth — the
   feedback and cases tables it syncs — so the desk sees what actually landed on it yesterday. */
const SIG_TYPE_NEG_FB = 'NEGATIVE FEEDBACK YESTERDAY';
const SIG_TYPE_CASES = 'CASES OPENED YESTERDAY';
const SIG_LAUNCH_TYPES = [SIG_TYPE_NEGATIVE, SIG_TYPE_WORST_CPC, SIG_TYPE_RETURNS, SIG_TYPE_NEG_FB, SIG_TYPE_CASES];
...
const SIG_COUNT_ONLY_TYPES = [SIG_TYPE_RETURNS, SIG_TYPE_NEG_FB, SIG_TYPE_CASES];
...
/* 18 Sept (owner): a loss item is an advertising/price matter — it no longer pages the CS desk. */
const SIG_ROLES_NEGATIVE = 'Management, Ops Head, Team Lead, Advertising Manager';
const SIG_ROLES_CS_DESK = 'Management, Ops Head, CS';
...
SIG_ACTIONS[SIG_TYPE_NEG_FB] = ['open feedback desk'];
SIG_ACTIONS[SIG_TYPE_CASES] = ['open case history'];
...
SIG_ROLES_FOR_TYPE[SIG_TYPE_NEG_FB] = SIG_ROLES_CS_DESK;
SIG_ROLES_FOR_TYPE[SIG_TYPE_CASES] = SIG_ROLES_CS_DESK;
...
const SIGNALS_CONFIG_DEFAULTS = {
  ...
  // 18 Sept — the CS desk's own signals (engine feedback + cases tables), per account per day
  signals_feedback_day_count: '2',        // negative/neutral feedback in one day that pages the CS desk
  signals_cases_day_count: '3',           // returns + INR + cases opened in one day
  signals_cases_multiple: '2',            // …or more than 2x the account's own 30-day daily rate
  signals_cases_min_day: '2',
...
function signalsThresholds_() {
  return {
    ...
    feedbackDayCount: Math.max(1, Math.round(signalsThreshold_('signals_feedback_day_count'))),
    casesDayCount: Math.max(1, Math.round(signalsThreshold_('signals_cases_day_count'))),
    casesMultiple: signalsThreshold_('signals_cases_multiple'),
    casesMinDay: Math.max(1, Math.round(signalsThreshold_('signals_cases_min_day'))),
...
/* 12 Sept (owner: "make detection also every 15 minutes"). computeSignals is a heavy scan (up to
 * SIG_SWEEP_BUDGET_MS of workbook reads); at a 15-minute cadence that would burn hours of Apps
 * Script execution a day and could exhaust the daily quota that EVERY portal action shares. The
 * safety valve: fingerprint the INPUTS first — Drive's lastUpdated of every linked workbook the
 * scan reads (central main sheet, order processing, sales analysis) plus the day list — and skip
  ...
  accounts.forEach(function (account) {
    ...
    ['central', 'order_processing', 'sales_analysis'].forEach(function (kind) {
      try {
        ...
        if (!id) { parts.push(account + '|' + kind + '|-'); return; }
...
function computeSignals(args) {
  ...
  signalsSeedConfig_();
  const force = !!(args && (args.force === true || String(args.force) === 'true'));
  ...
  const tm = { fp: 0, idx: 0, ret: 0, days: 0, cs: 0, tail: 0 };
  const tFp0 = Date.now();
  ...
  if (!force && fp === lastFp && (Date.now() - lastAt) < 3600000) {
    const skippedReport = 'unchanged — no linked workbook moved since ' + new Date(lastAt).toISOString().slice(11, 16) + 'Z; scan skipped';
    try { PropertiesService.getScriptProperties().setProperty('SIG_LAST_SKIP', new Date().toISOString() + ' ' + skippedReport); } catch (e) {}
  ...
  const skipped = [];
  const scannedFp = {};
  /* The per-account skip key leaves the order-processing workbook OUT: processors write today's
     order rows to it every few minutes through the shift (13:35 PKT: 5 of 6 accounts re-scanned,
     34 s, nothing new), while the scan only reads its 'Returns & INAD' tab for the days BEFORE
     today — a return logged today is dated today and is never in the judged window. A past-dated
     backfill reaches the hourly full pass, which still fingerprints all three workbooks. */
  const skipKey = function (a) {
    return String(fpMap[a] || '').split(';').filter(function (p) { return p.indexOf('|order_processing|') < 0; }).join(';');
  };
  ...
  // account; only the light Sales Analysis day tab is re-read per day. (The first backfill build read
  // both per account PER day, ~3x the heavy reads, which pinned Apps Script long enough for a
  // Cloudflare 524 and starved live users — owner, 8 Sept.)
  const found = [];
  const perDay = {};
  days.forEach(function (d) { perDay[d] = { day: d, scanned: 0, found: 0, notConnected: 0, diag: [] }; });
  let timedOut = false;
  ...
  for (let ai = 0; ai < accounts.length && !timedOut; ai++) {
    ...
    for (let di = 0; di < days.length; di++) {
      ...
      let sales;
      try { sales = signalsReadDayTab_(account, day); } catch (e) { sales = { ok: false, reason: String(e && e.message || e).slice(0, 60) }; }
      if (sales.ok) {
        touched = true;
        try { signalsSalesSignals_(account, day, th, sales, index, owners, found); } catch (e) { logActivity_('system', 'SIGNALS_SCAN_FAIL', account, '', '', day + ' sales ' + String(e && e.message || e)); }
        pd.diag.push({ account: account, tab: sales.tab, rows: (sales.rows || []).length, raised: found.length - before });
      } else {
        pd.notConnected++;
        pd.diag.push({ account: account, salesFail: sales.reason });
      }

      if (returns.ok) {
        touched = true;
        try { signalsReturnsSignals_(account, day, th, returns, index, owners, found); } catch (e) { logActivity_('system', 'SIGNALS_SCAN_FAIL', account, '', '', day + ' returns ' + String(e && e.message || e)); }
      } else if (di === 0) {
        pd.notConnected++;  // count the returns miss once per account, not once per day
      }
  ...
  /* 18 Sept: the CS desk's own signals — engine feedback + cases, once per hourly full pass (the
     engine syncs feedback hourly), never gated by a workbook fingerprint. */
  if (fullPass && !timedOut) {
    const tCs0 = Date.now();
    try {
      const facts = signalsEngineCsFacts_(days);
      if (facts.ok) { accounts.forEach(function (account) { days.forEach(function (day) { signalsCsSignals_(account, day, th, facts, found); }); }); }
      else logActivity_('system', 'SIGNALS_CS_FACTS_FAIL', '', '', '', facts.reason);
    } catch (e) { logActivity_('system', 'SIGNALS_SCAN_FAIL', '', '', '', 'cs ' + String(e && e.message || e).slice(0, 100)); }
    tm.cs = Date.now() - tCs0;
  }

  const tTail0 = Date.now();
  /* 18 Sept: every tail step is fenced and the engine mirror is pushed BEFORE the prune — from
     16 Sept 09:06 no run reached the push for 34 hours and the board froze on the 14th with
     nothing in any log to say why. A raise that fails is now logged and the mirror still moves. */
  let fresh = [];
  try { fresh = signalsRaise_(found, 'system'); } catch (e) { logActivity_('system', 'SIGNALS_RAISE_FAIL', '', '', '', String(e && e.message || e).slice(0, 120)); }
  const pushedRows = signalsPushEngine_(true);
  let pruned = 0;
  try { pruned = signalsPrune_(); } catch (e) { logActivity_('system', 'SIGNALS_PRUNE_FAIL', '', '', '', String(e && e.message || e).slice(0, 120)); }
  try { CacheService.getScriptCache().put(SIG_LAST_RUN_KEY, String(Date.now()), 21600); } catch (e) {}

  const order = days.map(function (d) { return perDay[d]; });
  const span = days.length > 1 ? days[days.length - 1] + '..' + days[0] : days[0];
  const summary = order.map(function (p) {
  ...
  const report = 'signals ' + span + ': ' + fresh.length + ' newly raised [' + summary + ']'
    + (pruned ? ', ' + pruned + ' old pruned' : '')
    + (skipped.length ? ', ' + skipped.length + ' unchanged account(s) skipped' : '')
    + ', mirror ' + (pushedRows < 0 ? 'PUSH FAILED' : pushedRows + ' rows')
    + ' t=fp ' + sec(tm.fp) + ' idx ' + sec(tm.idx) + ' ret ' + sec(tm.ret) + ' days ' + sec(tm.days) + ' cs ' + sec(tm.cs) + ' tail ' + sec(tm.tail)
    + (timedOut ? ', time budget hit — rest next run' : '')
    + (unhealthy ? ' | diag=' + JSON.stringify(order).slice(0, 1800) : '');
  signalsRememberRun_(props, report, fresh.length, pushedRows);
  return report;
}
/** 18 Sept: the engine calls this scan every quarter hour and throws its answer away, so a run
 * that died left no trace anywhere a person could read. The last report now lives in Script
 * Properties (signalsStatus shows it to Management) and every run writes one ACTIVITY_LOG line. */
function signalsRememberRun_(props, report, raised, pushed) {
  try { props.setProperty('SIG_LAST_SUMMARY', JSON.stringify({ at: new Date().toISOString(), text: String(report).slice(0, 900), raised: raised, pushed: pushed })); } catch (e) {}
  try { logActivity_('system', 'SIGNALS_RUN', '', '', String(raised), String(report).slice(0, 200)); } catch (e) {}
}
...
/* ---------- 18 Sept: the CS desk's own signals, from the engine ---------- */
/** One pull per run of the engine's feedback (negative/neutral only) and cases rows back to the
 * oldest scanned day minus the baseline window. backupDump pages in insertion order, so the
 * whole table is walked once (feedback ~6k rows = two pages) and only the window is kept. */
function signalsEngineCsFacts_(days) {
  const out = { ok: false, feedback: [], cases: [], reason: '' };
  try {
    const oldest = days[days.length - 1];
    const since = signalsAddDays_(oldest, -Math.max(1, Math.round(signalsThreshold_('signals_returns_baseline_days'))));
    const pull = function (table, dateOf, keep) {
      const rows = [];
      let offset = 0;
      for (let page = 0; page < 8; page++) {
        const d = enginePost_('backupDump', { table: table, offset: offset, limit: 4000 }) || {};
        const header = d.header || [], got = d.rows || [];
        got.forEach(function (r) {
          const o = {};
          header.forEach(function (h, i) { o[h] = r[i]; });
          if (String(dateOf(o) || '').slice(0, 10) >= since && keep(o)) rows.push(o);
        });
        if (d.done || !got.length) break;
        offset += got.length;
      }
      return rows;
    };
    out.feedback = pull('feedback', function (o) { return o.at; }, function (o) { return String(o.type) === 'Negative' || String(o.type) === 'Neutral'; });
    out.cases = pull('cases', function (o) { return o.opened_at; }, function () { return true; });
    out.ok = true;
  } catch (e) { out.reason = String(e && e.message || e).slice(0, 100); }
  return out;
}
/** Two account-level, count-only cards for one day: negative/neutral feedback (at or above the
 * day count) and cases opened (returns + INR + cases, a spike against the account's own rate). */
function signalsCsSignals_(account, day, th, facts, found) {
  const baseStart = signalsAddDays_(day, -th.baselineDays);
  const split = function (rows, dateOf) {
    const inDay = [], base = [];
    rows.forEach(function (r) {
      if (String(r.account || '') !== account) return;
      const when = String(dateOf(r) || '').slice(0, 10);
      if (when === day) inDay.push(r);
      else if (when >= baseStart && when < day) base.push(r);
    });
    return { day: inDay, base: base };
  };
  const fb = split(facts.feedback, function (r) { return r.at; });
  if (fb.day.length >= th.feedbackDayCount) {
    const rate = Math.round((fb.base.length / th.baselineDays) * 100) / 100;
    found.push(signalsCandidate_(account, SIG_TYPE_NEG_FB, day, { key: 'feedback', item_id: '', image: '' },
      fb.day.length + ' negative or neutral feedback', null, fb.day.length, rate, {
        feedback: fb.day.slice(0, 8).map(function (r) { return { buyer: String(r.buyer || ''), type: String(r.type || ''), text: signalsText_(r.text, 140), item_id: String(r.item_id || '') }; }),
        baseline_days: th.baselineDays, baseline_count: fb.base.length, baseline_per_day: rate,
      }));
  }
  const cs = split(facts.cases, function (r) { return r.opened_at; });
  if (cs.day.length) {
    const rate = cs.base.length / th.baselineDays;
    const spike = cs.day.length >= th.casesDayCount || (cs.day.length >= th.casesMinDay && cs.day.length > th.casesMultiple * rate);
    if (spike) {
      const kinds = {};
      cs.day.forEach(function (r) { const k = String(r.kind || 'CASE'); kinds[k] = (kinds[k] || 0) + 1; });
      found.push(signalsCandidate_(account, SIG_TYPE_CASES, day, { key: 'cases', item_id: '', image: '' },
        cs.day.length + ' cases opened', null, cs.day.length, Math.round(rate * 100) / 100, {
          cases: cs.day.slice(0, 8).map(function (r) { return { kind: String(r.kind || ''), buyer: String(r.buyer || ''), order_id: String(r.order_id || ''), item_id: String(r.item_id || ''), reason: signalsText_(r.reason, 80), status: String(r.status || '') }; }),
          kinds: signalsCounts_(kinds), baseline_days: th.baselineDays, baseline_count: cs.base.length, baseline_per_day: Math.round(rate * 100) / 100,
        }));
    }
  }
}
...
function signalsCacheCards_(db, found, stamp) {
  ...
  found.forEach(function (f) {
    ...
    /* 18 Sept: a card queued for APPEND in this run was marked at[k] = -1, and when the same item
       sat on a day tab twice (two rows, one title) the second copy took that -1 for a sheet row —
       getRange(-1, …) → "The starting row of the range is too small". Inside the raise, unfenced,
       so every quarter-hour scan died there from 16 Sept 09:21 and no signal moved for 34 hours.
       The first copy of a card wins; a row number below 1 is never a row. */
    if (at[k] === -1) return;
    if (at[k] > 0) {
...
function signalsMessage_(f) {
  ...
  if (f.type === SIG_TYPE_NEG_FB) {
    return '🟠 ' + f.value + ' negative or neutral feedback on ' + f.account + ' on ' + f.date + ' (its usual line is ' + f.baseline +
      ' a day). Read each one on the Feedback desk, reply, and ask for a revision where the buyer was helped.';
  }
  if (f.type === SIG_TYPE_CASES) {
    return '🟠 ' + f.value + ' cases opened on ' + f.account + ' on ' + f.date + ' (returns, item-not-received and disputes; its usual line is ' + f.baseline +
      ' a day). Work them the same day from the Returns and CS desks so none escalates.';
  }
...
function signalsPushEngine_(full) {
  try {
    ...
    for (let i = 0; i < rows.length || (i === 0 && full); i += 150) {
      enginePost_('syncSignals', { rows: rows.slice(i, i + 150), full: (full && i + 150 >= rows.length) ? 'true' : '', keys: full ? rows.map(function (x) { return x.skey; }) : [], pin_days: String(signalsThreshold_('signals_pin_days')) });
...
function slowActions(args) {
  ...
  for (let i = rows.length - 1; i >= 0; i--) {
    ...
    if (String(r.action) !== 'SLOW_ACTION') continue;
    const ms = Number(String(r.new_value || '').replace(/[^0-9]/g, '')) || 0;
    const a = String(r.target || '?');
    const b = by[a] || (by[a] = { n: 0, max: 0, sum: 0, last: '' });
...
/** 18 Sept: what the scan last reported — the run text with its timings and diag, when, and how
 * many rows reached the engine mirror. Management only; the answer to "are signals working?". */
function actionSignalsStatus_(payload, ctx) {
  if (!isMgmt_(ctx.user.role, ctx.ident.email)) throw new Error('SAY: Management only');
  const props = PropertiesService.getScriptProperties();
  let last = null;
  try { last = JSON.parse(props.getProperty('SIG_LAST_SUMMARY') || 'null'); } catch (e) { last = null; }
  return {
    last_run: last, last_skip: String(props.getProperty('SIG_LAST_SKIP') || ''),
    fingerprint_at: Number(props.getProperty('SIG_FP_AT') || 0) ? new Date(Number(props.getProperty('SIG_FP_AT'))).toISOString() : '',
    now: new Date().toISOString(), types: SIG_LAUNCH_TYPES,
  };
}

const ACTIONS_SIGNALS = {
  mySignals:         [actionMySignals_, 'any'],
  signalsStatus:     [actionSignalsStatus_, 'any'],
