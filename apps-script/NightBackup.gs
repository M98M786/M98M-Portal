...
function nbHeaderCols_(headers) {
  ...
  /* Return BOTH link columns. Preferring one is wrong: HAFIZA BHAJI has an empty 'New Ali Link'
   * and 8 filled rows in 'Ali Express Link' — the processors' real work was invisible because
   * the sweep looked only at the preferred-but-empty column. Each ROW takes whichever is set. */
  return { ebayCol: ebayCol, aliNumCol: aliNumCol, linkCol: linkCol, altLinkCol: aliLinkCol };
}
    ...
    for (var back = 0; back <= 6; back++) {
      ...
      for (var s = 0; s < sheets.length; s++) {
        ...
        for (var r = 1; r < values.length; r++) {
          ...
          if (!link && cols.altLinkCol >= 0) link = String(values[r][cols.altLinkCol] || '').trim();
          if (num.length < 8) num = '';
...
/** R8 (Hasib 25 Aug: "make speed fast of the working of tracking and other things").
 * The full aliSweep walks 7 days × every account — far too heavy for a 5-minute trigger. This
 * one reads ONLY today's tab per account, so an Ali number typed by a processor reaches the
 * portal within ~5 minutes instead of up to an hour. The hourly full sweep still backfills
 * anything this misses (a late edit on an older tab). */
function aliSweepFast() {
  var conns = readTab_('CONNECTIONS').filter(function (c) {
    return String(c.sheet_kind) === 'order_processing' && String(c.status || '').toLowerCase() !== 'off';
  });
  ...
  conns.forEach(function (c) {
    ...
    for (var s = 0; s < sheets.length; s++) {
      if (!ordersTabIsCandidate_(sheets[s].getName(), candidates)) continue;
      tabs++;
      /* COLUMN-LIMITED read (25 Aug): getDataRange() pulls every column of a populated day tab —
       * it cost 3s on empty tabs at dawn but 102s once the team filled them, and this rides the
       * 5-minute trigger. Read the header row, then ONLY the span of columns actually needed. */
      var sheet = sheets[s];
      ...
      for (var r = 0; r < span.length; r++) {
        var row = span[r];
        var id = String(row[cols.ebayCol - minC] || '').trim();
        if (!/^\d{2}-\d{5}-\d{5}$/.test(id)) continue;
        var num = cols.aliNumCol >= 0 ? String(row[cols.aliNumCol - minC] || '').replace(/\D/g, '') : '';
        var link = cols.linkCol >= 0 ? String(row[cols.linkCol - minC] || '').trim() : '';
        if (!link && cols.altLinkCol >= 0) link = String(row[cols.altLinkCol - minC] || '').trim();
        if (num.length < 8) num = '';
        if (link.indexOf('https://') !== 0) link = '';
        if (num || link) out.push({ order_id: id, ali_order: num, ali_link: link });
      }
      break;                                               // today's tab only
    }
  });
  var written = 0;
  for (var i = 0; i < out.length; i += 400) {
