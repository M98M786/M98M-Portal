...
(function () {
  ...
  /* Hasib item 13: the Seller Hub service-metrics panel — your INAD / INR rate against eBay's
     peer benchmark, per account. Green when you beat the peers, red when you trail them. */
  function ahServiceMetrics(rows) {
    if (!rows.length) {
      return '<div class="ah-note">eBay’s service metrics (item not as described / item not received, you vs peers) arrive with the nightly sync.</div>';
    }
    ...
    Object.keys(by).sort().forEach(function (acc) {
      ...
      by[acc].forEach(function (m) {
        var have = m.you != null;
        var worse = have && m.peer != null && Number(m.you) > Number(m.peer);
        h += '<tr' + (worse ? ' class="ah-std-bad"' : '') + '>' +
          '<td>' + esc(names[m.metric_type] || m.metric_type) + '</td>' +
          '<td class="ah-num">' + (have ? (Number(m.you)).toFixed(2) + '%' : '—') + '</td>' +
          '<td class="ah-std-thr">peers ' + (m.peer != null ? Number(m.peer).toFixed(2) + '%' : '—') +
            (m.count != null ? ' · ' + m.count + ' transactions' : '') + '</td>' +
          '<td>' + (have ? (worse ? '<span class="ah-lvl bad">ABOVE PEERS</span>' : '<span class="ah-lvl top">BEATING PEERS</span>') : '') + '</td></tr>';
      });
      h += '</tbody></table></div>';
    });
    return h + '</div>';
