...
(function () {
  ...
  function adRetry(msg, err, id) {
    return '<div class="ad-empty">' + esc(msg) + '<span>' + esc(err) + '</span>' +
  ...
  function adLoadPpc() {
    ...
    api('ppcTable', { account: AD.account, limit: AD.ppcLimit }).then(function (d) {
      ...
      if (ed) { ed.innerHTML = '<div class="ad-empty">The campaign editor needs the account&#39;s tab first.<span>' + esc(e.message) + '</span></div>'; }
    });
  }
