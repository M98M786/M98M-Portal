...
(function () {
  ...
  function huRunDupCheck(silent, done) {
    ...
    api('huntDuplicateCheck', { title: title, link: link }).then(function (d) {
      ...
      var m = (d && d.matches) || [];
      /* In revise mode the product ALWAYS matches the hunt being revised — that is not history,
         it is the very row being fixed. Warning about it forced a second click on every revision
         (9 Sept, Irfan). Other matches still warn. */
      if (HU_REVISING) { m = m.filter(function (x) { return huStr(x.hunt_id) !== HU_REVISING; }); }
      ...
      var rej = m.filter(function (x) { return x.status_label === 'rejected'; }).length;
  ...
  function huSubmit() {
    ...
    api(revId ? 'reviseHunt' : 'submitHunt', payload).then(function (res) {
      ...
      /* Two kinds of refusal. A VALIDATION refusal (a reason the hunter can act on) → restore the
         typing at once so they can fix it. A MAYBE-LANDED failure — timeout, overload, or the
         router's generic "request failed" (an internal error AFTER the row usually already
         landed) — must NOT dump the form back over a successful save (9 Sept, Irfan: "it says
         added, then it comes back on the whole form"). For those, VERIFY first: wait a few
         seconds for the server to finish, re-read the own list, and only restore if the save
         truly is not there. */
      var restore = function (note) {
      ...
      toast('The server hiccupped — checking whether your ' + (revId ? 'revision' : 'hunt') + ' saved…');
      var wantTitle = huNorm(payload.columns['Title'] || '');
      setTimeout(function () {
        api('myHunts', { status: 'all' }).then(function (d) {
          var hunts = (d && d.hunts) || [], landed = false, i;
          if (revId) {
            for (i = 0; i < hunts.length; i++) {
              if (huStr(hunts[i].hunt_id) === revId) { landed = huStr(hunts[i].approval_status) !== HU_REVISION; break; }
            }
          } else {
            for (i = 0; i < hunts.length; i++) {
              if (wantTitle && huNorm(huStr(hunts[i]['Title'])) === wantTitle) { landed = true; break; }
            }
          }
          if (landed) {
            if (typeof draftClear === 'function') { draftClear('hunting'); }
            toast(revId ? 'Saved ✓ — the revision landed and is back in the review queue.'
          ...
          /* Could not verify either way — keep the typing safe and say so honestly. A re-submit
             is safe: the server treats an own in-flight resubmit as a revision, never a dup. */
          restore('The server is slow — your typing is restored. Check "My hunts" first: it may have saved.');
        });
      }, 4000);
    });
  }
