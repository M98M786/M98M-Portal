...
(function () {
  ...
  function draw() {
    ...
    h += '<div class="at-panel"><div class="at-head"><div><div class="t">' + esc(H.title) + '</div>' +
      ...
      '<span class="at-tag">Stage: not computed yet (Phase 3)</span></div></div>' +
    ...
    /* where its ads sit */
    var C = D.campaigns || [];
    h += '<div class="at-panel"><h3>Where its ads sit right now' + src('campaign map, synced every 15 min') + '</h3><div class="at-sub">Campaign, type, campaign status and this listing\'s ad status inside it. Cost-per-sale ads carry no per-ad status on eBay; "exists" means live unless archived.</div>' +
    ...
    h += '<div class="at-grid2">' +
      '<div class="at-panel"><h3>Yesterday\'s behaviour' + src('orders') + src('sampled', true) + '</h3><div class="at-sub">' + esc(D.hourly_note) + '. Profile comparison arrives with Phase 3.</div><div id="atHour" class="at-chart"></div></div>' +
    ...
    h += '<div class="at-grid2">' +
      '<div class="at-panel"><h3>Weekday profile, last 30 days' + src('eBay ads report') + '</h3><div class="at-sub">Plain averages per weekday; shrinkage and the shuffle test arrive with Phase 3.</div><div id="atWd" class="at-chart short"></div>' +
      ...
      D.weekday.map(function (x) { return '<tr><td>' + x.day + '</td><td class="r">' + x.dates + '</td><td class="r">' + (x.units_day == null ? '—' : x.units_day) + '</td><td class="r">' + (x.attr_units_day == null ? '—' : x.attr_units_day) + '</td><td class="r">' + gbp(x.spend_day) + '</td><td class="r ' + cls(x.ad_profit_day) + '">' + pm(x.ad_profit_day) + '</td></tr>'; }).join('') + '</tbody></table></div>' +
    ...
    h += '<div class="at-grid2">' +
      '<div class="at-panel"><h3>Behaviour notes</h3><div class="at-note">Written by the analysis job from the numbers above, from Phase 7. Until then this panel stays empty rather than guessing.</div>' +
      '<h3 style="margin-top:12px">Today\'s decision preview</h3><div class="at-note">The Stop today engine (Phase 6, shadow mode) will show its proposal for this listing here.</div></div>' +
