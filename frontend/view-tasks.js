...
(function () {
  ...
  /* A loss review is the one type the server closes on an EXACT string: Tasks.gs rejects any
     submission note that is not one of these three, verbatim. My tasks only ever offered the
     free-text box every other type uses, so every loss review submitted since the type shipped
     was refused and 80 of them sat open while the advertising alert re-rang every 5 minutes.
     The three live here so the desk can only ever send a note the server accepts. */
  var TK_LOSS_RESOLUTIONS = ['Changed advertising', 'Changed price', 'Decision by management \u2014 keep same'];
  ...
  var TKV = { scope: 'mine', sel: {} };
  function tkIsMgmt() { return !!(STATE.user && (['Management', 'Ops Head'].indexOf(STATE.user.role) >= 0 || STATE.user.super)); }
  function tkSelIds() { return Object.keys(TKV.sel).filter(function (k) { return TKV.sel[k]; }); }
  function tkUpdateBulkBar() {
    var bar = $('tkBulkBar'); if (!bar) { return; }
    var ids = tkSelIds();
    if (!ids.length) { bar.className = 'tk-bulk hidden'; bar.innerHTML = ''; return; }
    bar.className = 'tk-bulk';
    bar.innerHTML = '<b style="margin-right:auto">' + ids.length + ' selected</b>' +
      '<button class="minibtn" data-bulk="end">End</button>' +
      '<button class="minibtn" data-bulk="extend">Extend deadline…</button>' +
      '<button class="minibtn" data-bulk="reassign">Reassign…</button>' +
      '<button class="minibtn" data-bulk="delete" style="border-color:var(--bad);color:var(--bad)">Delete</button>' +
      '<button class="minibtn" data-bulk="clear">Clear</button>';
  }
  /* Run one taskAdmin op across every selected task, in series, then reload once. */
  function tkBulkOp(op) {
    var ids = tkSelIds();
  ...
  VIEW_CSS.push(
    '.tk-bulk{position:sticky;top:8px;z-index:5;display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 12px;padding:10px 14px;border:1px solid var(--gold-a);border-radius:12px;background:var(--panel-2);box-shadow:0 6px 18px rgba(0,0,0,.25)}' +
    '.tk-bulk.hidden{display:none}' +
    '.tk-sel-cell{width:34px;text-align:center!important;padding-right:0!important}' +
    '.tk-sel-cell input,.tk-selall{width:16px;height:16px;cursor:pointer}' +
  ...
  VIEWS.tasks = {
    ...
    render: function () {
      return '<div class="hgroup enter d1"><h1>My tasks</h1>' +
        ...
        (tkIsMgmt() ? '<div id="tkBulkBar" class="tk-bulk hidden"></div>' : '') +
    ...
    init: function () {
      ...
      /* bulk selection (management): checkbox per row + select-all, driving the sticky bulk bar */
      var body = $('tkBody');
      if (body) {
        body.addEventListener('change', function (ev) {
          var t = ev.target, i, boxes;
          if (t && t.classList && t.classList.contains('tk-selall')) {
            boxes = body.querySelectorAll('input[data-tk-sel]');
            for (i = 0; i < boxes.length; i++) { boxes[i].checked = t.checked; TKV.sel[boxes[i].getAttribute('data-tk-sel')] = t.checked; }
            tkUpdateBulkBar(); return;
          }
          if (t && t.getAttribute && t.getAttribute('data-tk-sel')) {
            TKV.sel[t.getAttribute('data-tk-sel')] = t.checked; tkUpdateBulkBar();
          }
        });
      }
      var bulk = $('tkBulkBar');
      if (bulk) {
        bulk.onclick = function (ev) {
          var b = ev.target && ev.target.closest ? ev.target.closest('[data-bulk]') : null;
          if (!b) { return; }
          var op = b.getAttribute('data-bulk');
          if (op === 'clear') {
            TKV.sel = {};
            var boxes = document.querySelectorAll('input[data-tk-sel]'), i;
  ...
  function tkPaintTasks(box, d) {
    ...
    var selectable = tkIsMgmt();
    box.innerHTML = '<div class="scroll"><table class="tk-tbl">' +
      '<thead><tr>' + (selectable ? '<th class="tk-sel-cell"><input type="checkbox" class="tk-selall" title="Select all on this page"></th>' : '') +
        '<th>Task</th><th>Account</th><th>Item ID</th><th>Deadline</th><th>Status</th><th></th></tr></thead>' +
      '<tbody>' + tasks.map(function (t) { return tkRow(t, showWho, selectable); }).join('') + '</tbody></table></div>';
    tkWireRows(box);
    tkUpdateBulkBar();
  }
  ...
  function tkRow(t, showWho, selectable) {
    ...
    return '<tr class="tk-r">' +
        (selectable ? '<td class="tk-sel-cell"><input type="checkbox" data-tk-sel="' + tkAttr(id) + '"' + (TKV.sel[id] ? ' checked' : '') + '></td>' : '') +
      ...
      '</tr>' +
      (extra ? '<tr class="tk-x"><td colspan="' + (selectable ? 7 : 6) + '">' + extra + '</td></tr>' : '');
  }
  ...
  function tkSubmitForm(t, id, type) {
    ...
    if (type === 'loss_review') {
      return '<div class="tk-form hidden" data-form="' + tkAttr(id) + '">' +
        '<div class="field"><label>How was this loss resolved? Pick one \u2014 that is the whole submission.</label>' +
          '<div style="display:flex;flex-direction:column;gap:7px;margin-top:6px">' +
          TK_LOSS_RESOLUTIONS.map(function (r) {
            return '<label style="display:inline-flex;align-items:center;gap:7px;font-weight:600;font-size:12.5px;cursor:pointer">' +
              '<input type="radio" name="tkloss-' + tkAttr(id) + '" data-loss="' + tkAttr(id) + '" value="' + tkAttr(r) + '"> ' + esc(r) + '</label>';
          }).join('') + '</div>' +
          '<div class="tk-sub" style="margin-top:7px">The advertising alert for this item re-rings every 5 minutes until one of these three is recorded.</div></div>' +
  ...
  function tkRowAction(box, act, id, btn) {
    ...
    if (act === 'send') {
      ...
      /* loss_review carries a picked resolution, never typed prose: the server matches the note
         against its three strings exactly, so nothing may be prefixed or appended to it. */
      var lossPick = form ? form.querySelector('input[data-loss]:checked') : null;
      var isLoss = !!(form && form.querySelector('input[data-loss]'));
      if (isLoss) {
        if (!lossPick) { toast('Pick how the loss was resolved — one of the three.'); return; }
        btn.disabled = true; btn.textContent = 'Sent \u2713';
        if (form) { form.classList.add('hidden'); }
        toast('Sent for approval.');
        api('submitTask', { task_id: id, submission_note: tkStr(lossPick.value) })
          .then(function () { tkLoadTasks(); })
          ['catch'](function (e) {
            btn.disabled = false; btn.textContent = 'Send for approval';
            if (form) { form.classList.remove('hidden'); }
            toast('NOT submitted — ' + String((e && e.message) || ''));
          });
        return;
      }
