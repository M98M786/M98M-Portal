...
(function () {
  ...
  VIEW_CSS.push([
    ...
    '.sig-ackform{padding:6px 18px 12px}.sig-ackform.hidden{display:none}',
    '.sig-ackta{width:100%;min-height:54px;padding:9px 11px;border-radius:9px;border:1px solid var(--gold-line-hi);background:var(--panel);color:var(--text);font:inherit;font-size:12.5px;resize:vertical}',
  ...
  function sgActions(rec, i) {
    ...
    if (rec.acked_by_me) {
      out += '<span class="sig-todo" title="You already acknowledged this">Acknowledged by you</span>';
      return '<div class="sig-acts">' + out + '</div>';
    }
    out += '<button class="sig-ackb" data-sig-do="ackopen" data-sig-i="' + i + '">Acknowledge…</button>';
    /* 12 Sept (owner): an acknowledgement MUST carry a comment — the box opens under the card and
       the server refuses an empty one, so nobody clears a signal silently. */
    return '<div class="sig-acts">' + out + '</div>' +
      '<div class="sig-ackform hidden" data-sig-ackform="' + i + '">' +
        '<textarea class="sig-ackta" data-sig-note="' + i + '" rows="2" placeholder="What did you find or do about this? (required)"></textarea>' +
        '<div class="sig-acts" style="padding-top:8px"><button class="sig-ackb" data-sig-do="ack" data-sig-i="' + i + '">Acknowledge with this comment</button>' +
        '<button class="minibtn" data-sig-do="ackcancel" data-sig-i="' + i + '">Cancel</button></div>' +
      '</div>';
  }

  function sgWho(email) { var e = sgStr(email); return e.indexOf('@') > 0 ? e.slice(0, e.indexOf('@')) : e; }
  function sgFoot(rec, full) {
    var hist = rec.ack_history || [], j, out, h;
    if (!hist.length) {
      var acks = rec.acknowledged_by_others || [];
      if (!acks.length) { return ''; }
      hist = acks.map(function (a) { return { who: a, at: '', note: '' }; });
    }
    if (!full) {
      h = hist[hist.length - 1];
      return '<div class="sig-foot">' + esc(String(hist.length)) + ' acknowledged — latest ' + esc(sgWho(h.who)) +
        (h.note ? ': “' + esc(h.note) + '”' : '') + (rec.acked_by_me ? '' : ' · still pinned for you') + '</div>';
    }
    /* 12 Sept (owner): the history — who said what, in order. */
    out = '';
    for (j = 0; j < hist.length; j++) {
      h = hist[j];
      out += '<div class="sig-hist"><b>' + esc(sgWho(h.who)) + '</b>' +
        (h.at ? ' <span class="sig-when num">' + esc(sgStr(h.at).replace('T', ' ').slice(0, 16)) + '</span>' : '') +
        (h.note ? '<div class="sig-hist-note">“' + esc(h.note) + '”</div>' : '<div class="sig-hist-note sig-hist-none">no comment</div>') +
        '</div>';
    }
    return '<div class="sig-foot"><div class="k">Acknowledgement history · who said what</div>' + out + '</div>';
  }
  ...
  var SG_PAGE_LIST = null;   // set by a department page while it paints, so ack/go index into ITS list
  function sgClick(ev) {
    ...
    if (act === 'ackopen') { sgAckOpen(idx, el, true); return; }
    if (act === 'ackcancel') { sgAckOpen(idx, el, false); return; }
  ...
  function sgAckOpen(i, btn, open) {
    var host = btn && btn.closest ? btn.closest('.sig') : null;
    var form = host ? host.querySelector('[data-sig-ackform="' + i + '"]') : null;
    if (!form) { return; }
    form.className = 'sig-ackform' + (open ? '' : ' hidden');
    if (open) { var ta = form.querySelector('textarea'); if (ta) { ta.focus(); } }
  }
  function sgAck(i, btn) {
    var rec = (SG_PAGE_LIST || sgList())[i], key, host, ta, note;
    if (!rec) { return; }
    host = btn && btn.closest ? btn.closest('.sig') : null;
    ta = host ? host.querySelector('[data-sig-note="' + i + '"]') : null;
    note = ta ? sgStr(ta.value) : '';
    if (!note) { toast('Write a comment first — what did you find or do about this signal?'); if (ta) { ta.focus(); } return; }
    key = sgKey(rec);
    ...
    api('acknowledgeSignal', {
      account: rec.account, type: rec.type, date: rec.date, item_id: rec.item_id, note: note
    }).then(function (res) {
      delete SG.busy[key];
      if (SG_PAGE_LIST) {
        rec.acked_by_me = true; rec.pinned = false;
        (rec.ack_history = rec.ack_history || []).push({ who: (STATE.user && STATE.user.email) || 'you', at: (res && res.acknowledged_at) || '', note: note });
        var pg = document.getElementById('sigCsAll') ? ['sigCsAll', 'sigCsSum', 'CS', 'customer service'] : ['sigAdAll', 'sigAdSum', 'Advertising Manager', 'advertising'];
        sgDeptPaint(pg[0], pg[1], pg[2], pg[3]);
      }
      sgDrop(key);
      sgUnpin(key);
      ...
      delete SG.busy[key];
      btn.disabled = false;
      btn.textContent = 'Acknowledge with this comment';
  ...
  function sgAckAll(btn) {
    var note = window.prompt('Acknowledge EVERY signal on your board. One comment is required and will be recorded on each of them — what did you find or do?');
    if (note === null) { return; }
    note = sgStr(note);
    if (!note) { toast('A comment is required to acknowledge.'); return; }
    btn.disabled = true; var t = btn.textContent; btn.textContent = 'Clearing…';
    api('acknowledgeAllSignals', { note: note }).then(function (res) {
  ...
  function sgPaintAll(box) {
    ...
    // grouped by department (owner, 18 Sept) — each card keeps its index into sgList() for ack/go
    var groups = { 'Advertising Manager': [], 'CS': [], '': [] }, order = ['Advertising Manager', 'CS', ''], filled = 0, g, k, j;
    for (i = 0; i < list.length; i++) { groups[sgDeptOf(sgStr(list[i].type))].push(i); }
    for (k = 0; k < order.length; k++) { if (groups[order[k]].length) { filled++; } }
    for (k = 0; k < order.length; k++) {
      g = groups[order[k]];
      if (!g.length) { continue; }
      if (filled > 1) { out += '<div class="sig-dept-head">' + esc(SG_DEPT_LABEL[order[k]]) + ' <span class="num">' + g.length + '</span></div>'; }
      for (j = 0; j < g.length; j++) { out += sgCard(list[g[j]], g[j], true); }
    }
    box.innerHTML = out;
    sgWire(box);
  }
  ...
  VIEWS.signals = {
    ...
    render: function () {
      return '<div class="hgroup enter d1"><h1>Signals</h1>' +
          '<span class="sub">Pinned until you acknowledge them · grouped by department · Pakistan time</span>' +
  ...
  // ============================== department signal pages (owner, 12 Sept) ==============================
  /* "Make a separate page of customer service signals and a separate page of advertising signals;
     they should get updated every 5 minutes." Each page reads the same feed with include_acked so
     the acknowledgement HISTORY (who said what) stays visible after a signal is cleared, filters by
     the department token in targeted_roles, and re-fetches itself every 5 minutes while open.
     Signal DETECTION itself is the hourly engine run (heavy workbook scans) + the Refresh button;
     this timer keeps the page current with every acknowledgement and every new card within 5 min. */
  var SG_DEPT = { data: null, at: 0, timer: null, key: '' };
  var SG_DEPT_MS = 5 * 60 * 1000;
  /* 18 Sept (owner: "advertising, customer service and signals — all signal pages should have
     different signals; you are showing the same to everyone; they should receive signals related
     to their own departments"). A department page is a set of signal TYPES. The old filter used
     the department's ROLE TOKEN in targeted_roles — and the went-negative signal is raised to CS
     and the Advertising Manager together, so both desks were painted the same 47 cards. */
  var SG_DEPT_TYPES = {
    'Advertising Manager': ['WORST CPC PERFORMER YESTERDAY', 'WENT NEGATIVE YESTERDAY', 'Wrong Advertising'],
    'CS': ['RETURNS ABOVE USUAL', 'NEGATIVE FEEDBACK YESTERDAY', 'CASES OPENED YESTERDAY']
  };
  var SG_DEPT_LABEL = { 'Advertising Manager': 'Advertising', 'CS': 'Customer service', '': 'Other' };
  function sgDeptOf(type) {
    var k;
    for (k in SG_DEPT_TYPES) { if (SG_DEPT_TYPES.hasOwnProperty(k) && SG_DEPT_TYPES[k].indexOf(type) >= 0) { return k; } }
    return '';
  }
  function sgDeptLoad(fresh) {
    if (!fresh && SG_DEPT.data && Date.now() - SG_DEPT.at < 20000) { return Promise.resolve(SG_DEPT.data); }
    return api('mySignals', { include_acked: true }).then(function (d) { SG_DEPT.data = d || {}; SG_DEPT.at = Date.now(); return SG_DEPT.data; });
  }
  function sgDeptFilter(token) {
    var all = (SG_DEPT.data && SG_DEPT.data.signals) || [], types = SG_DEPT_TYPES[token] || [], out = [], i;
    for (i = 0; i < all.length; i++) {
      if (types.indexOf(sgStr(all[i].type)) >= 0) { out.push(all[i]); }
    }
    return out;
  }
  function sgDeptPaint(boxId, sumId, token, label) {
    var box = document.getElementById(boxId), sum = document.getElementById(sumId), list, out = '', i, pending = 0, acked = 0;
    if (!box) { return; }
    list = sgDeptFilter(token);
    for (i = 0; i < list.length; i++) { if (list[i].acked_by_me) { acked++; } else { pending++; } }
    if (sum) {
      sum.innerHTML = '<b class="num">' + pending + '</b> waiting on you · <b class="num">' + acked + '</b> acknowledged by you · ' +
        '<span>' + list.length + ' ' + esc(label) + ' signal' + (list.length === 1 ? '' : 's') + ' in the window</span>' +
        '<span class="sig-when num" style="margin-left:auto">updated ' + esc(new Date().toTimeString().slice(0, 5)) + ' · refreshes every 5 min</span>';
    }
    if (!list.length) {
      box.innerHTML = '<div class="card enter d1"><div class="hd">Nothing for ' + esc(label) + '</div><div class="bd"><div class="sig-empty"><b>No ' + esc(label) + ' signal in the current window.</b> New ones appear here within 5 minutes of the engine finding them.</div></div></div>';
      return;
    }
    // pending first, then acknowledged (history still shown on those)
    list.sort(function (a, b) { return (a.acked_by_me ? 1 : 0) - (b.acked_by_me ? 1 : 0); });
    SG_PAGE_LIST = list;
  ...
  function sgDeptInit(boxId, sumId, refreshId, token, label) {
    var box = document.getElementById(boxId), btn = document.getElementById(refreshId);
    if (SG_DEPT.timer) { clearInterval(SG_DEPT.timer); SG_DEPT.timer = null; }
    var paint = function () { if (document.body.contains(box)) { sgDeptPaint(boxId, sumId, token, label); } };
    var load = function (fresh) {
      return sgDeptLoad(fresh).then(paint)['catch'](function (e) {
        if (box && document.body.contains(box) && !(SG_DEPT.data)) { box.innerHTML = '<div class="sig-warnline">Signals could not be loaded — ' + esc(e.message) + '</div>'; }
      });
    };
    if (btn) { btn.onclick = function () { btn.disabled = true; btn.textContent = 'Refreshing…'; load(true).then(function () { btn.disabled = false; btn.textContent = 'Refresh'; }); }; }
    if (SG_DEPT.data) { paint(); }
    load(true);
    SG_DEPT.timer = setInterval(function () {
      if (!document.body.contains(box)) { clearInterval(SG_DEPT.timer); SG_DEPT.timer = null; SG_PAGE_LIST = null; return; }
      load(true);
    }, SG_DEPT_MS);
  }
  function sgDeptRender(title, sub, boxId, sumId, refreshId) {
    return '<div class="hgroup enter d1"><h1>' + title + '</h1>' +
        '<span class="sub">' + sub + '</span>' +
        '<button class="minibtn" id="' + refreshId + '" style="margin-left:auto">Refresh</button></div>' +
      '<div class="sig-dept-sum" id="' + sumId + '">Loading…</div>' +
      '<div class="sig-list" id="' + boxId + '"><div class="spinner"></div></div>';
  }
  VIEWS.csSignals = {
    label: 'CS signals',
    icon: '<path d="M12 4 3 19.5h18L12 4z"/><path d="M12 10.5v4M12 17.6h.01"/>',
    ...
    order: 0.3,
    render: function () { return sgDeptRender('Customer service <span class="goldtext">signals</span>', 'returns above usual, negative feedback and new cases for the CS desk · acknowledge with a comment · full history of who said what · refreshes every 5 minutes', 'sigCsAll', 'sigCsSum', 'sigCsRefresh'); },
    init: function () { sgDeptInit('sigCsAll', 'sigCsSum', 'sigCsRefresh', 'CS', 'customer service'); }
  };
  VIEWS.adSignals = {
    label: 'Advertising signals',
    icon: '<path d="M12 4 3 19.5h18L12 4z"/><path d="M12 10.5v4M12 17.6h.01"/>',
    roles: ['Advertising Manager', 'Management', 'Ops Head', 'Team Lead'],
    order: 0.3,
    render: function () { return sgDeptRender('Advertising <span class="goldtext">signals</span>', 'worst CPC performers, negative days and Wrong Advertising alarms · acknowledge with a comment · full history of who said what · refreshes every 5 minutes', 'sigAdAll', 'sigAdSum', 'sigAdRefresh'); },
    init: function () { sgDeptInit('sigAdAll', 'sigAdSum', 'sigAdRefresh', 'Advertising Manager', 'advertising'); }
  };
