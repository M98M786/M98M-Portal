...
(function () {
  ...
  function sgDeptFilter(token) {
    var all = (SG_DEPT.data && SG_DEPT.data.signals) || [], out = [], i, roles;
    for (i = 0; i < all.length; i++) {
      roles = sgStr(all[i].targeted_roles).split(',').map(function (t) { return t.trim(); });
      if (roles.indexOf(token) >= 0) { out.push(all[i]); }
    }
    return out;
  }
  ...
  VIEWS.csSignals = {
    ...
    order: 0.3,
    render: function () { return sgDeptRender('Customer service <span class="goldtext">signals</span>', 'negative-profit days and returns above usual for the CS desk · acknowledge with a comment · full history of who said what · refreshes every 5 minutes', 'sigCsAll', 'sigCsSum', 'sigCsRefresh'); },
