...
(function () {
  ...
  function odLoad() {
    ...
    var oc = cachedCall('todayOrders', payload, function (d) {
      ...
      odFetchPrefill(box, orders);  // remembered per-listing AliExpress links come back on new orders
  ...
  /* Owner, 10 Sept: "if a new aliexpress link is added for a listing, it comes back on another
     order of that specific listing." The engine remembers the link per ITEM; rows whose supplier
     cell is empty get it pre-filled in the paste box, and the purchase write carries it into the
     sheet's own Ali Express Link column. */
  function odFetchPrefill(box, orders) {
    try {
      if (typeof engineCall !== 'function') { return; }
      var want = [];
      orders.forEach(function (o) {
        if (!odStr(o[OD_H.aliLink]) && !odStr(o[OD_H.newAliLink]) && odStr(o[OD_H.orderNo])) { want.push(odStr(o[OD_H.orderNo])); }
      });
      if (!want.length) { return; }
      engineCall('orderPrefill', { order_ids: want }, 15000).then(function (d) {
        var pf = (d && d.prefill) || {};
        orders.forEach(function (o) {
          var hit = pf[odStr(o[OD_H.orderNo])];
          if (hit && safeUrl(hit.ali_link)) { o.__memLink = hit.ali_link; }
        });
        box.querySelectorAll('[data-od-alilink-in]').forEach(function (inp) {
          var hit = pf[inp.getAttribute('data-od-alilink-in')];
          if (hit && safeUrl(hit.ali_link) && !inp.value) {
            inp.value = hit.ali_link;
            inp.style.borderColor = 'var(--gold-a)';
            var note = document.createElement('span');
            note.className = 'od-note';
            note.style.cssText = 'color:var(--gold-a);font-weight:700;font-size:10.5px';
            note.textContent = 'remembered from a previous order of this listing — press Save link to stamp it';
            if (inp.parentNode) { inp.parentNode.appendChild(note); }
          }
        });
      }).catch(function () {});
    } catch (e) {}
  ...
  function odSend(box, btn, action, payload, row, slot, fields, keyed) {
    ...
    /* 10 Sept (owner: "accepted very fastly … in the sheet within seconds"): the engine takes the
       write first — its own record lands instantly, the v19 bridge stamps the day tab in seconds,
       and a bridge miss auto-retries every 15 minutes. Any engine hiccup falls straight back to
       the old sheet path, unchanged. */
    var call;
    if ((action === 'recordPurchase' || action === 'recordTracking') && typeof engineCall === 'function' && payload.order_number) {
      var o0 = odFind(row) || {};
      var fp = { account: payload.account, order_id: payload.order_number, tab: odStr(payload.tab) };
      ['cost', 'ali_order_number', 'email', 'new_ali_link', 'tracking_number', 'delivery_status'].forEach(function (k) {
        if (payload[k] !== undefined && payload[k] !== '') { fp[k] = payload[k]; }
      });
      if (!fp.delivery_status && payload.uploaded) { fp.delivery_status = 'Tracking'; }
      // the remembered main-supplier link rides along when the row's own cell is empty
      if (!odStr(o0[OD_H.aliLink]) && odStr(o0.__memLink)) { fp.ali_express_link = odStr(o0.__memLink); }
      call = engineCall('orderFastRecord', fp, 25000).then(function (r) {
        var sh = (r && r.sheet) || {};
        return { ok: true, shadow: !!sh.shadow, reason: odStr(sh.reason),
          skipped: [], unchanged: [], pendingSheet: !sh.ok && !sh.shadow };
      }).catch(function () { return api(action, payload); });
    } else {
    ...
    call.then(function (res) {
      ...
      if (res.pendingSheet) { unchanged += ' · sheet catching up — it lands automatically within minutes'; }
